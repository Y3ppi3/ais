...
class User(Base):
    ...
    password_hash = Column(String(128), nullable=False)  # используем имя, как в БД
...
class Payment(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    order_id = Column(Integer, ForeignKey("orders.id"), nullable=False)
    payment_method = Column(String(50), nullable=False)  # (карта, наличные, онлайн-кошелек и т.д.)
    payment_status = Column(String(50), default="pending")  # (успешен, отклонен, ожидание)
